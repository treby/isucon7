...
class App < Sinatra::Base
  configure do
    ...
    set :users_from_db, -> {
      ...
      @_users_from_db = db_client.query('SELECT id, name, display_name FROM user').to_a.each_with_object({}) do |row, hash|
        hash[row['id']] = row
      end
    }
    enable :sessions
  ...
  get '/message' do
    ...
    query = <<~SQL
      ...
      WHERE msg.id > ? AND msg.channel_id = ? ORDER BY msg.id DESC LIMIT 500
    SQL
